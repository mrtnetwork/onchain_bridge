...
#pragma warning(pop)
	...
	std::string OnChainBridge::RemoveKeyPrefix(const std::string &key)
	{
		return key.substr(ELEMENT_PREFERENCES_KEY_PREFIX_LENGTH);
	}
	std::optional<std::string> OnChainBridge::GetStringArg(
	...
	std::optional<std::vector<std::string>> OnChainBridge::GetStringListArg(
	...
	std::optional<std::vector<std::string>> OnChainBridge::GetListValueKey(const flutter::EncodableMap *args)
	{
	...
	std::optional<std::string> OnChainBridge::GetStringArgOrEmpty(
		const std::string &param,
		const flutter::EncodableMap *args)
	{
	...
	std::string OnChainBridge::NtStatusToString(const CHAR *operation, NTSTATUS status)
	{
	...
	DWORD OnChainBridge::GetApplicationSupportPath(std::wstring &path)
	{
	...
	std::wstring OnChainBridge::SanitizeDirString(std::wstring string)
	{
	...
	bool OnChainBridge::PathExists(const std::wstring &path)
	{
		struct _stat info;
	...
	bool OnChainBridge::MakePath(const std::wstring &path)
	{
	...
	PBYTE OnChainBridge::GetEncryptionKey()
	{
	...
	void OnChainBridge::Write(const std::string &key, const std::string &val)
	{
	...
	std::optional<std::string> OnChainBridge::Read(const std::string &key)
	{
		const DWORD NONCE_SIZE = 12;
		const DWORD KEY_SIZE = 16;
...
#pragma warning(pop)
	...
	flutter::EncodableMap OnChainBridge::ReadAll()
	{
	...
	void OnChainBridge::Delete(const std::string &key)
	{
		std::wstring appSupportPath;
		GetApplicationSupportPath(appSupportPath);
	...
	bool OnChainBridge::ContainsKey(const std::string &key)
	{
		std::wstring appSupportPath;
		GetApplicationSupportPath(appSupportPath);
	...
	bool OnChainBridge::LaunchUrl(const std::string &url)
	{
	...
	flutter::EncodableMap OnChainBridge::getPaths()
	{
	...
	HWND OnChainBridge::GetMainWindow()
	{
		return native_window;
	}

	void OnChainBridge::HandleFocusChange(bool hasFocus)
	{
	}
	...
	bool OnChainBridge::IsMaximized()
	{
	...
	bool OnChainBridge::SetMaximumSize(const flutter::EncodableMap &argsRef)
	{
	...
	bool OnChainBridge::SetMinimumSize(const flutter::EncodableMap &argsRef)
	{
		if (argsRef.find(flutter::EncodableValue("width")) != argsRef.end() &&
			argsRef.find(flutter::EncodableValue("height")) != argsRef.end())
		{
	...
	void OnChainBridge::SetAlwaysOnBottom(const flutter::EncodableMap &args)
	{
	...
	void OnChainBridge::ForceChildRefresh()
	{
	...
	void OnChainBridge::_EmitEvent(std::string eventName)
	{
	...
	void OnChainBridge::Hide()
	{
		ShowWindow(GetMainWindow(), SW_HIDE);
	}
	void OnChainBridge::Show()
	{
	...
	bool OnChainBridge::IsFullScreen()
	{
		return g_is_window_fullscreen;
	}
	bool OnChainBridge::IsMinimized()
	{
		HWND mainWindow = GetMainWindow();
		WINDOWPLACEMENT windowPlacement;
		GetWindowPlacement(mainWindow, &windowPlacement);
	...
	void OnChainBridge::SetFullScreen(const flutter::EncodableMap &args)
	{
		bool isFullScreen =
	...
	void OnChainBridge::WaitUntilReadyToShow()
	{
		::CoCreateInstance(CLSID_TaskbarList, NULL, CLSCTX_INPROC_SERVER,
						   IID_PPV_ARGS(&taskbar_));
	}
	void OnChainBridge::Unmaximize()
	{
		HWND mainWindow = GetMainWindow();
		WINDOWPLACEMENT windowPlacement;
		GetWindowPlacement(mainWindow, &windowPlacement);
	...
	bool OnChainBridge::IsPreventClose()
	{
		return is_prevent_close_;
	}
	void OnChainBridge::SetPreventClose(const flutter::EncodableMap &args)
	{

		is_prevent_close_ =
			std::get<bool>(args.at(flutter::EncodableValue("isPreventClose")));
	}
	void OnChainBridge::Close()
	{
		HWND hWnd = GetMainWindow();
		PostMessage(hWnd, WM_SYSCOMMAND, SC_CLOSE, 0);
	}
	void OnChainBridge::SetAsFrameless()
	{
		is_frameless_ = true;
		HWND hWnd = GetMainWindow();
	...
	flutter::EncodableMap OnChainBridge::GetBounds(
		const flutter::EncodableMap &args)
	{
		HWND hwnd = GetMainWindow();
		double devicePixelRatio =
	...
	void OnChainBridge::SetResizable(const flutter::EncodableMap &args)
	{
		HWND hWnd = GetMainWindow();
		is_resizable_ =
	...
	bool OnChainBridge::IsResizable()
	{
		return is_resizable_;
	}
	void OnChainBridge::Minimize()
	{
		HWND mainWindow = GetMainWindow();
		WINDOWPLACEMENT windowPlacement;
		GetWindowPlacement(mainWindow, &windowPlacement);
