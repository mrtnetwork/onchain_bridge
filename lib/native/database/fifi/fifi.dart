  ...
  Pointer<T> allocate<T extends NativeType>(int byteCount, {int? alignment}) {
    ...
    if (result.address == 0) {
      throw IDatabaseException.unexpected(
          'Could not allocate $byteCount bytes.');
    }
    return result;
  }
...
extension StringUtf8Pointer on String {
  ...
  Pointer<Utf8> toNativePointer({Allocator allocator = malloc}) {
    final units = StringUtils.encode(this);
    final result = allocator<Uint8>(units.length + 1);
    final nativeString = result.asTypedList(units.length + 1);
    nativeString.setAll(0, units);
...
extension BytesPointer on List<int> {
  Pointer<Uint8> toNativePointer({Allocator allocator = malloc}) {
    final result = allocator<Uint8>(length);
    final nativeString = result.asTypedList(length);
    nativeString.setAll(0, this);
    return result.cast();
  }

  Pointer<Uint16> toU16NativePointer({Allocator allocator = malloc}) {
    final result = allocator<Uint16>(length);
    final nativeString = result.asTypedList(length);
    nativeString.setAll(0, this);
    return result.cast();
  }
...
extension QuickPointer on Pointer {
